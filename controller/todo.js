...
async function get (request , reply){
    ...
    let data = await new Promise((resolve)=>
        connection.query(sql,[check.user_id],function(error,rows){
            if(error){
                console.log(error);
...
async function store (request , reply){
    ...
    let msg = data ? "Successfully added data!" : "Unable to add data successfully!";
...
async function update(request, reply) {
  ...
  let data = await new Promise((resolve) =>
    connection.query(
      sql,
      [title, description, created_at, updated_at, id, check.user_id],
      function (error, rows) {
        if (error) {
          console.log(error);
          return response.badRequest("", `${error}`, reply);
  ...
  let msg = data ? "Successfully changed data!" : "Unable to modify data!";
  return response.ok({}, msg, reply);
}
